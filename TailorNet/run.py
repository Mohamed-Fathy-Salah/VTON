...
def gen_body_gar(theta=get_specific_pose(0), beta=get_specific_shape('mean'), gender='female', upper_garment_class='t-shirt',lower_garment_class='short-pant'):
    save_body = True

    if(upper_garment_class != 'no_garment'):
        gen_gar(theta,beta,gender,upper_garment_class,"uppergar",save_body)
        save_body = False

    if(lower_garment_class != 'no_garment'):
        gen_gar(theta,beta,gender,upper_garment_class,"uppergar",save_body)
...
def gen_body_gar(theta=get_specific_pose(0), beta=get_specific_shape('mean'), gender='female', garment_class='short-pant'):
    ...
    pred_gar.write_obj_color(os.path.join(OUT_PATH, "pred_gar_{:04d}.obj".format(i)))
